...
Context Pack 以 `context.pack` 事件进入 RunEvidence、Runbook、诊断包和任务工
作台。`runtime/context_audit.py` 会把这些账本记录聚合成只读 Context Audit，用于
查看阶段、来源、信任度、新旧程度、记忆、历史任务候选和任务归属。它是可审计事实
包，不是任务路由器。
...
- 模型配置可以开启图片输入；如果接口或模型实际拒绝图片请求，模型流传输层应把
  视觉证据降级为文本事实后重试一次，并保留可审计事件。
...
- 每次视觉证据注入或文本降级都应写入 RunEvent/metadata，方便诊断“模型当时是否
  真的看到了图”。
...
- `runtime/context_pack.py`
  ...
  - 生成 Context Ledger。
- `runtime/context_audit.py`
  - 将 Context Ledger 汇总为任务工作台可展示的只读审计视图。
  - 不参与任务路由、工具选择、完成判断或风险拦截。
...
- `runtime/visual_verification.py`
  - 汇总截图、预览、调试会话、浏览器错误、验证形式和模型上下文注入事实。
  - 只生成 `visual_verification.v1` 证据摘要，不参与任务路由、工具选择或完成判断。
...
- 任务工作台提供 Context Audit 聚合视图，可直接看到上下文阶段、来源分布、历史记录、记忆记录、上下文卫生记录和任务 ID 缺口。
...
- RunResult、RunEvidence 和任务工作台都会展示 `visual_verification.v1` 摘要，用于审计是否有截图/渲染、是否存在 console/page/network 错误，以及是否已进入模型上下文。
